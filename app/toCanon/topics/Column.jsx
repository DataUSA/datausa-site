...
export default class TextViz extends Component {
  ...
  render() {
    ...
    return <div className={ `topic ${slug} ${ comparisons.length ? "compare" : "" } Column` }>
      { data.map((d, i) => <div key={i} className="topic-body">
        { d.title &&
          <h3 className="topic-title">
            {!i ? <a href={ `#${ slug }`} id={ slug } className="anchor" dangerouslySetInnerHTML={{__html: d.title}}></a> : <span>{ d.title }</span>}
          </h3>
        }
